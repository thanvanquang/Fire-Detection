...
from utils.general import (LOGGER, check_file, check_imshow, colorstr,
...
@torch.no_grad()
def run(weights=ROOT / 'yolov5s.onnx',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        pred = session.run(output_names, {input_name: im})
        t3 = time_sync()
        dt[1] += time_sync() - t2
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                det[:, 0:4] = scale_coords(im.shape[2:], det[:, 0:4], im0.shape).round()
    ...
    results = pd.DataFrame(
        columns=['video', 'technique', 'threshold', 'window_size', 'detected', 'first_frame', 'time_avg'])
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'weights/yolov5l.onnx', help='model path(s)')
    parser.add_argument('--source', type=str, default='/media/ubuntu/DATA/FACENETv1/DETECTION/Fire/Video/demo/6.mp4',
    ...
    parser.add_argument('--temporal', type=str, help='temporal analysis technique used after detection',
                        default='persistence')
    parser.add_argument('--area-thresh', type=float, default=0.05,
    ...
    parser.add_argument('--persistence-thresh', type=float, default=0.50,
